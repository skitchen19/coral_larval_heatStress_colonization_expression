...
working.dir <- ("C:/Users/Sheila's Comp/Documents/motifs/2022_repeat")
...
for (i in 1:length(list.files)){
 ...
 oligo$module<-rep(ifelse(grepl("_M6_",list.files[i]),"M6", 
                            ifelse(grepl("_M8_",list.files[i]),"M8",
                                   ifelse(grepl("_M11_",list.files[i]),"M11",
                                          ifelse(grepl("_M13_",list.files[i]),"M13",
                                                 ifelse(grepl("_M15_",list.files[i]),"M15",
                                                        ifelse(grepl("_M18_",list.files[i]),"M18",
                                                               ifelse(grepl("_M19_",list.files[i]),"M19","M20"))))))),nrow(oligo))
...
for (i in 1:length(mat.files)){
  # Read the files
  ncol <- max(count.fields(mat.files[i], sep = "\t"))
  mat = read.table(mat.files[i], header = F, comment.char = ";", sep="\t",fill=T,col.names=paste0('V', seq_len(ncol)))
  ...
  mat$module<-rep(ifelse(grepl("_M6_",mat.files[i]),"M6", 
                         ifelse(grepl("_M8_",mat.files[i]),"M8",
                                ifelse(grepl("_M11_",mat.files[i]),"M11",
                                       ifelse(grepl("_M13_",mat.files[i]),"M13",
                                              ifelse(grepl("_M15_",mat.files[i]),"M15",
                                                     ifelse(grepl("_M18_",mat.files[i]),"M18",
                                                            ifelse(grepl("_M19_",mat.files[i]),"M19","M20"))))))),nrow(mat))
...
for (i in 1:length(dyad.files)){
  ...
  dyad$module<-rep(ifelse(grepl("_M6_",dyad.files[i]),"M6", 
                          ifelse(grepl("_M8_",dyad.files[i]),"M8",
                                 ifelse(grepl("_M11_",dyad.files[i]),"M11",
                                        ifelse(grepl("_M13_",dyad.files[i]),"M13",
                                               ifelse(grepl("_M15_",dyad.files[i]),"M15",
                                                      ifelse(grepl("_M18_",dyad.files[i]),"M18",
                                                             ifelse(grepl("_M19_",dyad.files[i]),"M19","M20"))))))),nrow(dyad))
...
for (i in 1:length(mat.dyad)){
  ...
  mat_dyad$module<-rep(ifelse(grepl("_M6_",mat.dyad[i]),"M6", 
                              ifelse(grepl("_M8_",mat.dyad[i]),"M8",
                                     ifelse(grepl("_M11_",mat.dyad[i]),"M11",
                                            ifelse(grepl("_M13_",mat.dyad[i]),"M13",
                                                   ifelse(grepl("_M15_",mat.dyad[i]),"M15",
                                                          ifelse(grepl("_M18_",mat.dyad[i]),"M18",
                                                                 ifelse(grepl("_M19_",mat.dyad[i]),"M19","M20"))))))),nrow(mat_dyad))
...
for (i in 1:length(annot_oligo)){
  ...
  at<-annt %>% dplyr::arrange(desc(Ncor)) %>% 
    group_by(X.id1) %>% dplyr::slice(1)
  ...
  at$module<-rep(ifelse(grepl("_M6_",annot_oligo[i]),"M6", 
                        ifelse(grepl("_M8_",annot_oligo[i]),"M8",
                               ifelse(grepl("_M11_",annot_oligo[i]),"M11",
                                      ifelse(grepl("_M13_",annot_oligo[i]),"M13",
                                             ifelse(grepl("_M15_",annot_oligo[i]),"M15",
                                                    ifelse(grepl("_M18_",annot_oligo[i]),"M18",
                                                           ifelse(grepl("_M19_",annot_oligo[i]),"M19","M20"))))))),nrow(at))
...
annt_data = as.data.frame(do.call(rbind, datalist3[-20]))

all.data.annt <- all.data %>% 
  left_join(select(annt_data,"X.id1","name1","name2","cor", "Ncor", "interval", "strand", "module"),by = c("name1"="X.id1", "interval", "module"))

write.table(all.data.annt, "Adig_modules_motifs_combined_2022Update.txt", sep="\t")
